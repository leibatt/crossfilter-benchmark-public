...
def run(dbFilename):
  ...
  cursor.execute("""
        ...
        """)
  loadData("movies",conn,os.path.join(os.path.dirname(__file__),'..','..','dataset_movies_10M_fixed.csv'))
  ...
  cursor.execute("""
        ...
        WIND DOUBLE)
        """)
  loadData("weather",conn,os.path.join(os.path.dirname(__file__),'..','..',"dataset_weather_10M_fixed.csv"))
  ...
  cursor.execute("""
        CREATE TABLE if not exists flights(
            ...
            FL_DATE TEXT)
        """)
  loadData("flights",conn,os.path.join(os.path.dirname(__file__),'..','..',"dataset_flights_10M.csv"))
  conn.commit()
  conn.close()
